...
from src.product_search.clip_supervised_engine import (  # noqa: E402
    build_clip_articletype_model,
    configure_lightweight_finetune,
    get_clip_train_eval_transforms,
    get_trainable_parameters,
)
from src.product_search.config import (  # noqa: E402
    CLIP_ARTICLETYPE_BEST_CHECKPOINT,
    CLIP_ARTICLETYPE_LAST_CHECKPOINT,
...
TRAIN_LOG = REPORT_DIR / "clip_articletype_training_log.csv"
    ...
    torch.save(
        {
            ...
            "base_clip_checkpoint": str(LOCAL_CLIP_CHECKPOINT),
...
def main():
    parser = argparse.ArgumentParser(description="Lightweight fine-tune CLIP image encoder for articleType supervision.")
    ...
    train_tf, eval_tf = get_clip_train_eval_transforms(args.image_size)
    ...
    lora_like_info = configure_lightweight_finetune(model, unfreeze_visual_blocks=args.unfreeze_visual_blocks)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(get_trainable_parameters(model), lr=args.lr, weight_decay=args.weight_decay)
    scaler = torch.amp.GradScaler("cuda", enabled=device.type == "cuda")

    start_epoch = 1
    best_val_top1 = -1.0
    best_val_top5 = -1.0
    if args.resume and CLIP_ARTICLETYPE_LAST_CHECKPOINT.exists():
        ckpt = torch.load(CLIP_ARTICLETYPE_LAST_CHECKPOINT, map_location=device, weights_only=False)
        ...
        print(f"resumed_from={CLIP_ARTICLETYPE_LAST_CHECKPOINT}")
    ...
    train_config.update(lora_like_info)
    ...
    print(f"trainable_params={lora_like_info['trainable_params']}")
    print(f"total_params={lora_like_info['total_params']}")
    print(f"trainable_ratio={lora_like_info['trainable_ratio']:.6f}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_metrics = run_epoch(
            ...
            progress_label=f"clip-ft train epoch {epoch}/{args.epochs}",
        )
        val_metrics = run_epoch(
            ...
            progress_label=f"clip-ft val epoch {epoch}/{args.epochs}",
        )
        ...
        save_checkpoint(CLIP_ARTICLETYPE_LAST_CHECKPOINT, model, optimizer, epoch, best_val_top1, best_val_top5, mapping, args, train_config)
        if is_best:
            save_checkpoint(CLIP_ARTICLETYPE_BEST_CHECKPOINT, model, optimizer, epoch, best_val_top1, best_val_top5, mapping, args, train_config)
        if (
            args.checkpoint_every > 0
            and epoch % args.checkpoint_every == 0
            and not train_config["is_smoke_test"]
        ):
            periodic_checkpoint = CLIP_ARTICLETYPE_LAST_CHECKPOINT.with_name(f"clip_articletype_epoch_{epoch:03d}.pth")
        ...
        log_row = {
            ...
            "trainable_ratio": lora_like_info["trainable_ratio"],
        }
        append_log(log_row)
        print(
    ...
    print(f"last_checkpoint={CLIP_ARTICLETYPE_LAST_CHECKPOINT}")
    print(f"best_checkpoint={CLIP_ARTICLETYPE_BEST_CHECKPOINT}")
