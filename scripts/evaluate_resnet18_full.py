...
METRICS_JSON = REPORT_DIR / "resnet18_full_test_metrics.json"
PER_CLASS_CSV = REPORT_DIR / "resnet18_full_per_class_accuracy.csv"
...
def upsert_summary(metrics):
    ...
    if SUMMARY_CSV.exists():
        ...
        df = df[df["model_name"] != "resnet18_full_dataset"]
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate ResNet18 full dataset classifier on test split.")
    ...
    if not RESNET18_FULL_BEST_CHECKPOINT.exists():
        raise FileNotFoundError("Run scripts/train_resnet18_full.py before evaluation.")
